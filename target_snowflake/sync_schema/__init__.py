...
def convert_jsonschema_types_to_lists(jsonschema: dict) -> dict:
  """Walk the JSON Schema definition, converting any type definitions that are not lists to lists."""
  ...
  for keyword in ['oneOf', 'allOf', 'anyOf']:
    if keyword in jsonschema:
      new_schema[keyword] = [convert_jsonschema_types_to_lists(defn) for defn in jsonschema[keyword]]
...
class BaseSchemaSync(PluginBase, metaclass=abc.ABCMeta):
    ...
    def sync_table(self, stream_name: str, key_properties: List[str], jsonschema: dict) -> Dict[str, str]:
      ...
      if not existing_schema:
        self.logger.info(f"Creating table '{table_name}' for stream '{stream_name}' with key properties: {key_properties}")
        self.create_table(table_name, key_properties, schema)
        return schema
      
      for column_name, type in schema.items():
        if not column_name in existing_schema:
          self.logger.info("Adding column '{column_name}' to table '{table_name}'")
          ...
          self.logger.info(f"Column '{column_name}' on table '{table_name}' changed from {existing_schema[column_name]} to {type}. Versioning column by renaming it to '{new_column_name}' and adding a new column in it's place.")
          self.rename_column(table_name, column_name, new_column_name)
          self.add_column(table_name, column_name, type)
        else:
          self.logger.debug(f"Table '{table_name}' for stream '{stream_name}' already exists with the proper schema")
        
      return schema

    def jsonschema_to_table_schema(self, jsonschema: dict) -> Optional[Dict[str, str]]:
      """Convert a JSON Schema definition of a stream to a map of column names to their SQL data types."""
      ...
      return {
        self.property_name_to_column_name(name): self.jsonschema_to_sql_type(definition)
    ...
    def jsonschema_to_sql_type(self, jsonschema: dict) -> str:
      """Convert the given JSON Schema property definition to a compatible SQL data type."""
      pass
    ...
    @abc.abstractmethod
    def create_table(self, table_name: str, key_properties: List[str], schema: Dict[str, str]) -> None:
