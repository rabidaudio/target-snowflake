...
class PostgresSchemaSync(BaseSchemaSync):
  ...
  def get_table(self, table_name: str) -> Optional[Dict[str, str]]:
    res = self.query("SELECT column_name, data_type FROM information_schema.columns " + 
  ...
  def add_column(self, table_name: str, column_name: str, type: str) -> None:
    self.execute(SQL("ALTER TABLE {}.{} ADD COLUMN {} " + type).format(
  ...
  def rename_column(self, table_name: str, old_name: str, new_name: str) -> None:
    self.execute(SQL("ALTER TABLE {}.{} RENAME COLUMN {} to {}").format(
      Identifier(self.schema), Identifier(table_name), Identifier(old_name), Identifier(new_name)
    ))
  ...
  def jsonschema_to_sql_type(self, jsonschema: dict) -> str:
    if 'array' in jsonschema['type'] or 'object' in jsonschema['type']:
      return 'jsonb'
    if (
      'format' in jsonschema and
      jsonschema['format'] == 'date-time' and
      'string' in jsonschema['type']
    ):
      return 'timestamp with time zone'
    if (
      'format' in jsonschema and
      jsonschema['format'] == 'date' and
      'string' in jsonschema['type']
    ):
      return 'date'
