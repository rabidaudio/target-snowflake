...
def test_create_table(
    db: psycopg2.extensions.connection, schema_sync: PostgresSchemaSync
):
...
def test_add_column(
    ...
    with db.cursor() as cur:
        cur.execute(
            ...
            + "(id bigint, name character varying, PRIMARY KEY (id))"
        )
    ...
    with db.cursor() as cur:
        cur.execute(
            ...
            + "and column_name='email' and data_type='character varying' LIMIT 1"
        )
        res = cur.fetchall()
        assert len(res) == 1
...
def test_alter_column(
    ...
    with db.cursor() as cur:
        cur.execute(
            "CREATE TABLE test_schema.users (id bigint, age bigint, PRIMARY KEY (id))"
        )
    ...
    with db.cursor() as cur:
        cur.execute(
            "SELECT 1 FROM information_schema.columns "
            + "where table_schema='test_schema' and table_name='users' "
            + "and column_name='age' and data_type='numeric' LIMIT 1"
        )
        res = cur.fetchall()
        assert len(res) == 1
    ...
    with db.cursor() as cur:
        cur.execute(
            "SELECT 1 FROM information_schema.columns "
            + "where table_schema='test_schema' and table_name='users' "
            + "and column_name='age_20210920_0745' and data_type='bigint' LIMIT 1"
        )
        res = cur.fetchall()
        assert len(res) == 1
